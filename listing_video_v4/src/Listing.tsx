// Listing.tsx — Schoolhouse v5.2 composition
// 1080×1920 portrait, 30fps, 117s total
// 21 photo beats (was 23: dropped workshop barn + sheep+cattle per Matt)
// Family rockpile beat now uses wide multi_point_pan to traverse all faces
//
...
const FPS = 30;
export const LISTING_TOTAL_SEC = 117.0;
...
const BEATS: BeatDef[] = [
  ...
  { photo: 'v5_library/historic/03_william_p_with_cane.jpg',
    ...
    historic: true },
  // Family rockpile — wide multi_point_pan traversing ALL faces (Matt v5.1 fix)
  // Anchors: x=-22 → 0 → +22 with intensity 1.0 = ~840px traverse over 7s
  { photo: 'v5_library/historic/09_family_rockpile.jpg',
    ...
    move: { move: 'multi_point_pan', focal: 'center', intensity: 1.0,
      anchors: [{x:-22,y:0,scale:1.12},{x:0,y:0,scale:1.14},{x:22,y:0,scale:1.12}] },
    historic: true },

  // === Act 2: Ranch life (2 beats, brief) =====================================
  // Dropped workshop_barn + sheep_with_cattle per Matt — keep surrey + sheep_dip
...
export const Listing: React.FC = () => {
  const musicVolume = (frame: number) => {
    ...
    if (t < 86) return 0.20;
    if (t < 87) return interpolate(t, [86, 87], [0.20, 0.45]);
    if (t < 112) return 0.45;
    return interpolate(t, [112, 114], [0.45, 0.0], { extrapolateRight: 'clamp' });
  };

  const VO = [
    { src: 'audio/v51_s01.mp3', startSec: 7.0 },     // Beats 1+2 family
    { src: 'audio/v51_s02.mp3', startSec: 19.5 },    // Beat 3 rockpile (mid-pan)
    { src: 'audio/v52_s03.mp3', startSec: 26.0 },    // Beats 4-5 ranch life (NEW shorter)
    { src: 'audio/v51_s04.mp3', startSec: 37.5 },    // Beat 6 bridge
    { src: 'audio/v51_s05.mp3', startSec: 42.5 },    // Beats 7-8 architect+entry
    { src: 'audio/v51_s06.mp3', startSec: 53.0 },    // Beats 9-11 modern intro
    { src: 'audio/v51_s07.mp3', startSec: 79.2 },    // Beat 15 sunroom
    { src: 'audio/v51_s08.mp3', startSec: 87.5 },    // Beat 17 fire patio
    { src: 'audio/v51_s09.mp3', startSec: 92.7 },    // Beat 18 elk
    { src: 'audio/v51_s10.mp3', startSec: 100.7 },   // Beat 20 river
    { src: 'audio/v51_s11.mp3', startSec: 105.2 },   // Beat 21 closing line
  ];
  ...
  return (
    <AbsoluteFill style={{ background: CHARCOAL }}>
      ...
      <Sequence from={Math.round(109 * FPS)} durationInFrames={Math.round(5 * FPS)}>
        <RevealInner durationSec={5} />
      </Sequence>

      <BrandOutro startFrame={Math.round(114 * FPS)} durationSec={3} />
    </AbsoluteFill>
  );
};
