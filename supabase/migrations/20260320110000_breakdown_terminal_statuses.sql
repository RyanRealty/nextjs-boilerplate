-- By-city breakdown: add withdrawn, expired, canceled, and finalized (count with history_finalized).
-- Terminal statuses (closed, expired, withdrawn, canceled): we do not re-fetch listing data; we can backfill history.
...
BEGIN
  ...
  -- by_city: total, active, pending, closed, withdrawn, expired, canceled, other
  WITH city_rows AS (
    SELECT
