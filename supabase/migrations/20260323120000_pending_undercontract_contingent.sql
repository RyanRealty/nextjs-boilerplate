-- Count ActiveUnderContract (one word) and Contingent as pending.
-- Some MLSs send "ActiveUnderContract" or "Contingent" instead of "Active Under Contract" or "Pending".
...
BEGIN
  ...
  -- Pending = pending, under contract (with or without space), undercontract, contingent
  WITH city_rows AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE
        ...
        OR LOWER(COALESCE(l."StandardStatus", '')) LIKE '%contingent%'
      )::int AS pending,
...
COMMENT ON FUNCTION refresh_listings_breakdown() IS 'Recomputes breakdown. Pending = pending, under contract, undercontract, contingent. Call after every sync.';
