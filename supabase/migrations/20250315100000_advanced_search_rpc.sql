-- Advanced search: single RPC that filters on flat columns + details (jsonb).
-- Enables filters for YearBuilt, LotSizeAcres, garage, pool, view, waterfront, keywords, open house, etc.
...
CREATE OR REPLACE FUNCTION search_listings_advanced(
  ...
  p_status_filter text DEFAULT 'active',  -- 'active' | 'active_and_pending' | 'pending' | 'closed' | 'coming_soon' | 'all'
  p_keywords text DEFAULT NULL,          -- ilike on PublicRemarks
  ...
  p_new_listings_days int DEFAULT NULL,  -- listed in last N days (ModificationTimestamp)
  p_sort text DEFAULT 'newest',          -- newest | oldest | price_asc | price_desc | price_per_sqft_asc | price_per_sqft_desc | year_newest | year_oldest
...
COMMENT ON FUNCTION search_listings_advanced IS 'Advanced listing search: flat columns + details jsonb (YearBuilt, LotSizeAcres, garage, pool, view, waterfront, keywords, open house). Used by /listings and /search advanced filters.';
