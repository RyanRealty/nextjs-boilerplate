-- Include "Under Contract" (RESO: Active Under Contract) in pending and active_and_pending filters.
-- Pending-only filter: Pending OR Under Contract. Active: exclude those so "Active Under Contract" is not counted as active.
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (p_status_filter IS NULL OR p_status_filter = 'all'
        OR (p_status_filter = 'active' AND (l."StandardStatus" IS NULL OR ((l."StandardStatus" ILIKE '%Active%' OR l."StandardStatus" ILIKE '%For Sale%' OR l."StandardStatus" ILIKE '%Coming Soon%') AND l."StandardStatus" NOT ILIKE '%Pending%' AND l."StandardStatus" NOT ILIKE '%Under Contract%')))
        OR (p_status_filter = 'active_and_pending' AND (l."StandardStatus" IS NULL OR l."StandardStatus" ILIKE '%Active%' OR l."StandardStatus" ILIKE '%For Sale%' OR l."StandardStatus" ILIKE '%Coming Soon%' OR l."StandardStatus" ILIKE '%Pending%' OR l."StandardStatus" ILIKE '%Under Contract%'))
        OR (p_status_filter = 'pending' AND (l."StandardStatus" ILIKE '%Pending%' OR l."StandardStatus" ILIKE '%Under Contract%'))
