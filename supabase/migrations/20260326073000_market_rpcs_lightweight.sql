-- Phase 1 follow-up: lightweight aggregation to avoid statement timeouts
...
begin
  ...
  with closed_sales as (
    select
      ...
      coalesce(("details"->>'DaysOnMarket')::numeric, ("details"->>'CumulativeDaysOnMarket')::numeric) as dom,
  ...
  base as (
    select
      ...
      avg(dom) as avg_dom,
      avg(case when living_area > 0 then close_price / living_area else null end) as avg_ppsf,
