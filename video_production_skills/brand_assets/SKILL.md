...
1. **Authenticity over polish.** Raw, real, personal > clean corporate animation. The most-shared real estate content on Instagram looks handmade, not templated.
2. **Data is the hook.** Stats are the magnet. Visuals serve the stat, not the other way around. Every piece of content must teach or reveal something.
3. **Platform-native always.** Content should look like it belongs in the feed, not like an ad dropped in. If a viewer can tell it's "produced," the creative failed.
...
- Single bold typographic statement that implies a story
- Unexpected compositional choice that creates visual tension
- Color combo that signals premium (navy + white = authority, gold = prestige)

**Loses in 50ms:**
- Multiple competing elements
- Text overlaid on busy image areas (illegible)
- Neutral, forgettable color schemes
- Generic layout the brain has seen a thousand times
...
### Typography (mandatory hierarchy)
...
### Access path (Canva-embedded photos)

Rich (Framed Visuals Co) delivers photos via Aryeo → Drive. Fastest route is through Canva designs that already have them embedded:

1. `search-designs` query = address
2. `start-editing-transaction` on each matching design ID
3. Transaction response includes asset_id values for each page element
4. `get-design-thumbnail` per page → visual evaluation
5. **Cancel** the transaction (read-only, no edits)
6. Build asset ID catalog by purpose
...
### Photo evaluation criteria by purpose

#### EXTERIOR HERO (main listing post, Reel opener, FB cover)
- Architecture clearly visible (roofline, facade, entrance)
- Property scale apparent (lot/land visible)
- No distractions (no cars, trash cans, harsh shadows)
...
- Mountains / landmark geography CLEARLY visible (not just a hint)
...
- Natural light
- Furniture livable, not over-staged
- Flow to adjacent rooms or outdoors
- ❌ Reject: cramped, dark, over-staged
...
1. **Lighting:** golden hour > soft overcast > bright midday > harsh shadows
2. **Emotional impact at thumbnail size:** if you can feel it at 300px wide, it works on mobile
3. **Story value:** does this ADD something the listing copy doesn't already say?
4. **Technical:** sharpness, horizon level, no distortion
5. **Uniqueness:** the photo that shows something no other photo in the set shows

For hero slot (one image tells the whole story): for luxury acreage, the photo showing BOTH structure AND land/view wins every time. Viewer should recognize the property type in 0.5s.

### Tumalo Reservoir Rd asset ID reference (canonical worked example)

Rich's delivered photos for 19496 Tumalo Reservoir Rd, $1,599,000, 3BR/2.5BA, 2.28 ac, MLS 220193552:
...
- Hero image: `MAHFnwjB4lQ`
- Views emphasis: `MAHFn4IpIrg`
- Lifestyle / warmth: `MAHFn_YbEAw`
- Interior variety: `MAGsyRAbxIE`

---

## Scroll-stop techniques

### Pattern interrupts (1.7s scroll decision window)
...
- Question that implies the viewer is wrong ("Think you can't afford Bend?")

**Tier 2 (rotation):**
- Unexpected visual texture (macro nature, abstract data viz, aerial Cascades)
- Bold typography on dark (oversized white on navy/black)
- Stat with emoji or symbol accent (📈 or ↑)

**Tier 3 (functional fallback):**
- Color pop (single bright element on monochrome)
- Motion in frame 1 (subtle zoom from frame 0)

### Thumbnail / first-frame strategy

The first frame IS the thumbnail. Design as standalone:
...
## Format-platform fit matrix

| Format | IG Reels | TikTok | FB Reels | YT Shorts |
|--------|----------|--------|----------|-----------|
| Stat card sequence | ★★★ | ★★★ | ★★★ | ★★★ |
| Market commentary | ★★★ | ★★ | ★★★ | ★★★ |
| Listing showcase | ★★★ | ★★ | ★★ | ★★ |
...
★★★ = primary fit · ★★ = works with adaptation · ★ = low fit

---

## Creative defaults (when in doubt)

| Element | Default |
|---------|---------|
| Background (video) | Dark navy gradient (`#102742` → `#0a1929`) |
| Background (static) | White or off-white |
| Text color | White (`#ffffff`) hero on dark; navy on white |
| Accent | Gold `#c9a84c` |
| Font (video) | Amboqia headlines, AzoSans body |
| Font (static) | Butcher headlines, AzoSans body, Amboqia accent |
| Motion | Scale-up text reveal, 1.03× Ken Burns on backgrounds |
...
## Real-estate creators worth studying (verified 2026-04-17)

| Creator | Handle | Bench | Steal | Skip |
|---------|--------|-------|-------|------|
| Mike Hege | @mikehege (Charlotte) | 85M-view viral Reel | Authenticity, stat-driven hooks, doesn't take himself too seriously | Talking-head (we don't have Matt on camera regularly) |
| Lisa Dubois | (Falls Creek, VA) | 12M-view listing video | Creative concepts that make properties memorable | Heavy on-camera presence |
| Michael Perry | @michaelperry (SLC) | #1 team in Utah | Visual quality bar, cinematic composition, professional but warm | Team-focused (we're a solo brand) |

### Faceless / data-driven account principles
